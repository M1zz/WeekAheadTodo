...
## ✅ 심플 화면 기한(데드라인) 편집 (2026-06-16)
- [x] 각 할 일 줄에 기한 항상 표시 (오늘/내일/날짜 + 달력 아이콘) — 오늘이면 숨기던 동작 수정
- [x] 동그라미=완료 토글 / 제목 탭=기한 변경으로 분리, VoiceOver 기본동작+커스텀동작(기한 변경/삭제)
- [x] 기한 변경 시트(DeadlineEditSheet) — 빠른 버튼(오늘/내일/이번 주말) + 달력(graphical) → updateTask 저장
- [x] 추가 시트에도 달력 날짜 선택(QuickDatePicker) 추가 — 정확한 데드라인 지정 가능
